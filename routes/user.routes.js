...
router.post('/register', async (req, res) => {
  try {
    ...
    const existingUser = await client.db("CRM").collection("users").findOne({ email });
    if (existingUser) {
    ...
    // Create a new user
    const user =  await client.db("CRM").collection("users").insertOne({
      email,
...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await client.db("CRM").collection("users").findOne({ email });
    if (!user) {
